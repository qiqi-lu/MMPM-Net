"""
Model based methods for parameter mapping.
######
...
Unconstrained problem argmin(x) ||A(x)-b||_2^2 + lambda_1*||D(x)||_1
...
def model_mono_exp_r2(x,tau):
    '''s = m*exp(-tau*p)
    '''
    m,p = np.split(x,2,axis=-1)
    s   = m*np.exp(-tau*p)
    return s

def costfun_mono_exp_r2(c,x,y):
    return y-c[0]*np.exp(-x*c[1])

def nlls_mono_exp_r2(s,tau,**kwargs):
    smax = np.max([np.max(s),0.0])
    c0   = np.array([smax,1.0])
    bounds = ([0.0, 0.0], [3.0, 10.0])
    res = least_squares(costfun_mono_exp_r2, c0, args=(tau,s), bounds = bounds,method='trf')
    return [res.x[0],res.x[1]]

def invalid_model(x):
    raise Exception('Invalid Model.')

def pixel_wise_mapping(s,tau,method='nlls',model_type='mono_exp_r2',pbar_leave=True,ac=4):
    models = {'nlls_mono_exp_r2': nlls_mono_exp_r2}
    model  = models.get('{}_{}'.format(method,model_type),invalid_model)
    c  = s.shape[-1]
    sv = np.reshape(s,(-1,c))
    n  = sv.shape[0]
    xv = []
    ...
    global fun
    def fun(i):
        x = model(s=sv[i],tau=tau)
        return (i,x)
        
    pool = multiprocessing.Pool(ac)
    pbar = tqdm.tqdm(total=n,desc=model_type,leave=pbar_leave)
    for xk in pool.imap_unordered(fun,range(n)):
        xv.append(xk)
        pbar.update(1)
    pool.close()
    pbar.close()

    xv = sorted(xv,key=operator.itemgetter(0))
    xv = [x[1] for x in xv]
    xmap_shape = list(s.shape[:-1])
    xmap_shape.append(-1)
    xmap = np.reshape(np.stack(xv),newshape=xmap_shape)
    return xmap


if __name__ == "__main__":
    x = np.ones(shape=(10,32,32,2))
    m,p = np.split(x,2,axis=-1)
    a = list(x.shape[:-1])
    a.append(-1)
    print(a)
